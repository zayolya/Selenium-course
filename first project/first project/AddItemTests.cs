...
namespace first_project
{
    ...
    public class AddItemTests : TestBase
    {
        ...
        {
            LogintoAdminPanel();
            Driver.Url = "http://localhost:81/litecart/admin/?app=catalog&doc=catalog";
            ...
            Driver.FindElements(By.CssSelector("input[type=checkbox][name^=categories]"))[1].Click();
            HiddenSelectOptionSelect(By.Name("default_category_id"), "1");
            Driver.FindElements(By.CssSelector("input[type=checkbox][name^=product_groups]"))[1].Click();
            FillInput(By.CssSelector("input[name=quantity]"), "1");

            UploadFile(By.Name("new_images[]"), @"C:\Users\olgaz\Desktop\new.JPG");

            Driver.FindElement((By.CssSelector("input[name=date_valid_from]"))).SendKeys("02/20/2016");
            Driver.FindElement((By.CssSelector("input[name=date_valid_to]"))).SendKeys("02/28/2016");
            ...
            Assert.IsTrue(IsElementPresent(By.XPath("//table[@class='dataTable']//a[contains(.,'" + itemName + "')]")));
        }
    }
}
